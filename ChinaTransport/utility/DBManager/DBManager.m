...
//      kuaidi
-(NSMutableArray *)loadkuaidiData:(NSString *)charc{
    FMResultSet *result=[fm executeQuery:@"select * from kuaidi where  substr(idxChar,1,1) = ? ",charc];
...
-(NSMutableArray *)loadkuaidiTopIdxData{
    FMResultSet *result=[fm executeQuery:@"select * from kuaidi where topIdx > 0"];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
        NSString *idxchar=[result stringForColumn:@"idxChar"];
        NSString *name=[result stringForColumn:@"name"];
...
-(NSMutableArray *)loadtrainData:(NSString *)charc{
    FMResultSet *result=[fm executeQuery:@"select * from train_station  where  substr(sta_code_first,1,1) = ? ",charc];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
...
-(NSMutableArray *)loadregionProvinceData{
    FMResultSet *result=[fm executeQuery:@"select * from region where level = 1"];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
        NSString *name=[result stringForColumn:@"name"];
...
-(NSMutableArray *)loadregionCityData{
    FMResultSet *result=[fm executeQuery:@"select * from region where level = 2"];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
        NSString *name=[result stringForColumn:@"name"];
        NSString *level=[result stringForColumn:@"level"];
...
-(NSMutableArray *)loadWeizhangProvinceData{
    FMResultSet *result=[fm executeQuery:@"select   * from weizhangcity where level = 1"];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
        NSString *name=[result stringForColumn:@"name"];
        NSString *level=[result stringForColumn:@"level"];
...
-(NSMutableArray *)loadweizhangCityData{
    FMResultSet *result=[fm executeQuery:@"select * from weizhangcity where level = 2 "];
    NSMutableArray *dataArray=[NSMutableArray arrayWithCapacity:0];
    while ([result next]) {
        NSString *name=[result stringForColumn:@"name"];
        NSString *level=[result stringForColumn:@"level"];
...
-(void)updateIsRead:(DrivingModel *)drivingModelType{
    
    [fm executeUpdate:@"UPDATE drivingexam SET isRead = '1' WHERE id = '3'",(long)drivingModelType.drivingID];
