...
//                model.classa=@"0";
                ...
                if ([responseobj[@"classa"]isEqualToString:@"1"]) {
                    ChaWeizhanModel *model =[[ChaWeizhanModel alloc] init];
                    model.isenabled =responseobj[@"isenabled"];
                    model.engine =@"0";
...
-(void)createUI{
    ...
    weizhangTopTableView.bounces = NO;
    [self.view addSubview:weizhangTopTableView];
//      验证码

    UILabel *randLabel =[MyControl createLabelWithtext:@"验证码" font:16 textcolor:RGBCOLOR(68, 68, 68) backgroundColor:nil];
    [self.view addSubview:randLabel];
    randTextField =[[UITextField alloc] init];
    randTextField.placeholder=@"点击图片刷新";
...
//    randTextField.layer.cornerRadius =8;
    randTextField.font = [UIFont systemFontOfSize:14];
    ...
    [randLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(weizhangTopTableView.mas_bottom).with.offset(20);
    }];
    [randTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(weizhangTopTableView.mas_bottom).with.offset(20);
        make.left.equalTo(randLabel.mas_right).with.offset(42);
//        make.right.equalTo(self.view.mas_centerX).with.offset(55);
        make.size.mas_equalTo(CGSizeMake(100, 20));
        make.height.equalTo(randLabel.mas_height);
    }];
    [randImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(weizhangTopTableView.mas_bottom).with.offset(19);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row<2) {
        ...
        NSLog(@"cell.textLabel.font:%@",cell.textLabel.font);
        if (indexPath.row ==0) {
            ...
            carButton =[MyControl createButtonWithType:UIButtonTypeCustom Frame:CGRectMake(wid-70, 20, 50, 20) title:abbrStr titleColor:RGBCOLOR(51, 51, 51) imageName:@"证件" bgImageName:nil target:self method:nil];
            carButton.titleEdgeInsets = UIEdgeInsetsMake(0, 3, 0, 0);
            [cell addSubview:carButton];
            carNumField =[MyControl createTextFieldFrame:CGRectMake(110, 20, 100, 20) placeholder:nil bgImageName:nil leftView:nil rightView:nil isPassWord:NO];
            carNumField.placeholder =@"请输入车牌号";
            carNumField.font = [UIFont systemFontOfSize:14];
